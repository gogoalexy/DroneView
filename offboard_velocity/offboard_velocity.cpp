...
bool offb_ctrl_body(std::shared_ptr<mavsdk::Offboard> offboard)
{
    ...
    Offboard::VelocityBodyYawspeed fw{};
    fw.forward_m_s  = 1.0f;
    offboard->set_velocity_body(fw);
    sleep_for(seconds(5));
    
    offboard_log(offb_mode, "Backward");
    Offboard::VelocityBodyYawspeed bw{};
    bw.forward_m_s  = -2.0f;
    offboard->set_velocity_body(bw);
    sleep_for(seconds(5));
